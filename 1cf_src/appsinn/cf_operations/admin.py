...
class AttendanceRecordAdmin(MultitenantAdminMixin, BaseAdmin):
    ...
    def generate_report_view(self, request):
        ...
        events = (
            ...
            .order_by("-modified_at", "title")[:200]
        )
        sessions = (
            EventSession.objects.for_user(request.user)
            .select_related("event", "branch", "branch__organization")
            .order_by("event__title", "sort_order")[:200]
        )
        zones = (
            Zone.objects.for_user(request.user)
            .select_related("branch", "branch__organization")
            .order_by("branch__name", "name")[:200]
        )
        subgroups = (
            SubBranch.objects.for_user(request.user)
            .select_related("zone", "branch", "branch__organization")
            .order_by("branch__name", "zone__name", "name")[:300]
        )
        ...
        filters = {
            ...
            "subgroup": (request.GET.get("subgroup") or "").strip(),
    ...
    def import_excel_view(self, request):
        ...
        from cf_users.models import Branch

        branches = Branch.objects.for_user(request.user).order_by(
            "organization__name", "name"
        )
        events = (
            Event.objects.for_user(request.user)
            .select_related("branch")
            .order_by("-modified_at", "title")[:100]
        )
        context = {
            **self.admin_site.each_context(request),
            "opts": self.model._meta,
        ...
        if request.method == "POST":
            ...
            if event_id:
                event = (
                    Event.objects.for_user(request.user)
                    .filter(pk=event_id, branch=branch)
                    .first()
                )
                if event is None:
                    messages.error(
