...
class MultitenantAdminMixin:
    ...
    @property
    def media(self):
        from cf_utils.admin_autocomplete import autocomplete_media

        return super().media + autocomplete_media()
...
class MultitenantOrgFilter(admin.SimpleListFilter):
    """Filter by Organisation — Django admin autocomplete (tenant-scoped)."""

    title = _("organisation")
    parameter_name = "organization"
    template = "admin/cf_utils/autocomplete_filter.html"
    ...
    def lookups(self, request, model_admin):
        ...
        if org is None:
            return []
        return [(str(org.pk), str(org))]

    def autocomplete_config(self):
        selected_pk = self.value() or ""
        selected_label = ""
        for lookup, title in self.lookup_choices:
            if str(lookup) == str(selected_pk):
                selected_label = str(title)
                break
        return {
            "app_label": "cf_users",
            "model_name": "branch",
    ...
    def queryset(self, request, queryset):
        ...
        if not path:
            for parent_name in ("record", "department", "group", "event"):
                path = resolve_organization_filter_path(
...
class MultitenantBranchFilter(admin.SimpleListFilter):
    """Filter by Branch — Django admin autocomplete (tenant-scoped)."""
    ...
    def lookups(self, request, model_admin):
        ...
        if not value:
            return []
        Branch = apps.get_model("cf_users", "Branch")
        branch = Branch.objects.filter(pk=value).select_related("organization").first()
        if branch is None:
            return []
        return [(str(branch.pk), str(branch))]
